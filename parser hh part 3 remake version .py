...
cnt_docs = len(os.listdir(r'C:\Users\Get over it\Desktop\python\docs\vacancies'))
i = 0

# Проходимся по всем файлам в папке vacancies
for fl in os.listdir(r'C:\Users\Get over it\Desktop\python\docs\vacancies'):

    # Открываем, читаем и закрываем файл
    f = open(r'C:\Users\Get over it\Desktop\python\docs\vacancies/{}'.format(fl), encoding='utf8')
    jsonText = f.read()
    f.close()
    ...
    # skills_name.append(jsonObj['key_skills'])
    # зашарпал то, что снизу и добавил сюда нейм
    # добавленное тоже зашарпил, потому что ошибка
...
# Эту штуку я зашарпил, чтобы поменять:
# eng = sql.create_engine('postgresql://{Пользователь}:{Пароль}@{Сервер}:{Port}/{База данных}')
# пример из документации:  postgresql://scott:tiger@localhost/mydatabase
eng = sql.create_engine('postgresql://postgres:QWERTY@localhost/hh2')
conn = eng.connect()
